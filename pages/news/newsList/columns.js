...
const invoiceStatusObj = {
  ...
  if (row.currentImageAddressTumb && row.currentImageAddressTumb !== 'Not-set')  {
    return <Avatar className='me-50' img={row.currentImageAddressTumb} width='32' height='32' />}
...
export const columns = [
  {
    ...
    minWidth: '140px',
    // selector: row => row.id,
    cell: row =><div className='d-flex'><div className='me-1'>{getUserAvatar(row)}</div>
      <Link to={`/apps/invoice/preview/${row.id}`}>{row.title}</Link></div>
  },
  {
    name: 'نام نویسنده',
    sortable: true,
    ...
    minWidth: '107px',
    // selector: row => row.id,
    cell: row => <Link to={`/apps/invoice/preview/${row.id}`}>{`${row.newsCatregoryName}`}</Link>
  },
  // {
  //   sortable: true,
  //   minWidth: '102px',
  ...
  // },
  {
    ...
    cell: row => <span>{row.updateDate}</span>
  },
  // {
  //   sortable: true,
  //   minWidth: '200px',
